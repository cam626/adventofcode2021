...
def bfs(node, adjList, visited, doubleNode):
    if node == "end":
        return 1

    if node in visited:
        if doubleNode is not None or node == "start":
            return 0
        
        numPaths = 0
        for childNode in adjList[node]:
            numPaths += bfs(childNode, adjList, visited, node)
    ...
    numPaths = 0
    for childNode in adjList[node]:
        numPaths += bfs(childNode, adjList, newVisited, doubleNode)
...
def bfsDriver(adjList):
    ...
    return bfs("start", adjList, visited, None)
